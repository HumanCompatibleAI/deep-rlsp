...
from deep_rlsp.solvers.value_iter import value_iter
from deep_rlsp.rlsp import compute_d_last_step


def sample_from_posterior(
    env, s_current, p_0, h, temp, n_samples, step_size, r_prior, gamma=1, print_level=1
):
    """
    ...
    if r_prior is None:
        r = 0.01 * np.random.randn(env.num_features)
    else:
        r = 0.1 * r_prior.rvs()
