...
from deep_rlsp.envs.gridworlds.train import TrainState
...
TRAIN_PROBLEMS = {
    ...
    "default": (
        TrainSpec(
            5,
            5,
            TrainState((0, 4), {(1, 2): True}, (2, 1), True),  # init state
            [(4, 0)],  # carpet
            [(2, 0), (4, 2)],  # goals
            # train transitions
            {(2, 1): (3, 1), (3, 1): (3, 2), (3, 2): (2, 2), (2, 2): (2, 1)},
        ),
        TrainState((2, 0), {(1, 2): True}, (2, 2), True),  # current state for inference
        np.array([0, 0, 0, 0, 0, 0, 0, 0, 1]),  # specified reward
        np.array([-1, 0, -1, 0, 0, 0, 0, 0, 1]),  # true reward
    )
}
