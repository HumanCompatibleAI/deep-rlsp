import numpy as np
from deep_rlsp.envs.gridworlds.batteries import BatteriesState
...
def get_problem(version):
    ...
    spec = BatteriesSpec(
        ...
        BatteriesState(
            (0, 4),  # agent pos
            (2, 1),  # train pos
            8,  # train battery (/life)
            {(0, 0): True, (4, 4): True},  # batteries present in environment
            False,  # agent carrying battery
        ),
        [(2, 0), (4, 2)],  # goals
        # train transitions
        {(2, 1): (3, 1), (3, 1): (3, 2), (3, 2): (2, 2), (2, 2): (2, 1)},
    )
    final_state = BatteriesState(
        (2, 0),  # agent pos
        (3, 2),  # train pos
        8,  # train battery (/ life)
        {(0, 0): False, (4, 4): True},  # batteries present in environment
        False,  # agent carrying battery
    )
...
BATTERIES_PROBLEMS = {"default": get_problem("default"), "easy": get_problem("easy")}
