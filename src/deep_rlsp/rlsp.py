...
def compute_feature_expectations(mdp, policy, p_0, T):
    ...
    for t in range(T - 2, -1, -1):
        ...
        future_features = future_features * np.expand_dims(policy[t], axis=-1)
...
def rlsp(
    ...
    def compute_grad(r_vec):
        ...
        if solver == "value_iter":
            ...
            policy = ppo.learn(
                r_vec, reset_model=reset_solver, total_timesteps=solver_iterations
            )
        ...
        G = compute_g(
            mdp, policy, p_0, horizon, d_last_step_list, expected_features_list
        )
    ...
    for i in range(epochs):
        ...
        with np.printoptions(precision=4, suppress=True):
            print(
                "Epoch {}; Reward vector: {}; grad norm: {}".format(i, r_vec, grad_norm)
            )
            if check_grad_flag:
